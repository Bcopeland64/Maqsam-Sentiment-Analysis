# model_conversion_complete.py
# First, install required libraries if they're not already installed
# Uncomment and run these lines first if you're missing dependencies
"""
import subprocess
import sys
...
install("sentencepiece")  # Required for XLMRobertaTokenizer
"""
...
# --- Configuration ---
model_name = "microsoft/Multilingual-MiniLM-L12-H384" # Example model name
...
try:
    # First try with slow tokenizer
    tokenizer = AutoTokenizer.from_pretrained(
        model_name,
        use_fast=False  # Use slow tokenizer since we have sentencepiece installed
    )
    logger.info("Successfully loaded slow tokenizer")
except Exception as e:
    logger.exception(f"Error loading slow tokenizer: {e}")
    logger.info("Please make sure 'sentencepiece' is installed with: pip install sentencepiece")
    raise
...
try:
    ...
    reloaded_tokenizer = AutoTokenizer.from_pretrained(
        onnx_model_dir,
        use_fast=False  # Use slow tokenizer for verification too
    )
